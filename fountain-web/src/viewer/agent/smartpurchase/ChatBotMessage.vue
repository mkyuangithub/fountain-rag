<template>
  <div class="chat-message" :class="{ 'user-message': isUser }">
    <div v-if="props.message && props.message.length > 0" class="message-content">
  ...
  -->
    ...
    <div v-if="goodsList && goodsList.length > 0">
      <div style="margin-left: 100px;">
        <span style="font-size: 13px; color: #595959; margin-bottom: 10px;">
          以下是您感兴趣的商品:
        </span>
      </div>
      <div class="goods-display">
        <div v-for="item in goodsList" :key="item.id" class="goods-item">
          <img :src="item.pictureUrl || '/assets/images/no-image.png'" :alt="item.name" class="goods-image">
...
const goodsList = computed(() => {
  if (props.isData && props.productList) {
...
.goods-item:hover .goods-image {
  ...
  border-color: #1890ff; /* 悬浮时边框变为更深的蓝色 */
}
