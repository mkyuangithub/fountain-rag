    ...
    <div class="display-content" ref="chatMessageContainer">
      ...
      <div style="margin: 5px 5px 5px 5px">
        <chat-bot-message v-for="(message, index) in messages" :key="index" :is-user="message.isUser"
          :message="message.content" :chat-type="m01" :is-data="message.isData" :productList="message.productList" />
      </div>
      ...
      <div style="margin: 0; padding: 0; margin-bottom: 0px">
        <div class="textarea-wrapper" style="margin: 0; padding: 0">
          <a-textarea v-model:value="userPrompt" placeholder="请输入您的问题，注意不要输入敏感词内容（政治相关、色情、血腥、暴恐等）。"
            :auto-size="{ minRows: 3, maxRows: 3 }" :disabled="isThinking" class="user-prompt" />
        </div>
        <div style="width: 100%; padding: 0; margin-top:0px;">
          <div
            ...
            <div
              style="margin-top: 10px;  margin-left: 5px; display: flex; align-items: center; justify-content: center; justify-items: center;">
              ...
              <a-button type="primary" :disabled="isThinking" class="submit-btn" @click="handlePicUpload"
                style="background-color: #5694F6;  margin-left: auto; margin-right: 5px; margin-top:3px;">
                <template #icon>
                  <FileAddOutlined />
                </template>
          ...
          <div v-if="uploadedPicUrl"
            style="display: flex; justify-content: right; margin-right: 5px; margin-top: 10px;">
            <img :src="uploadedPicUrl" style="height: 100px; width: auto;" />
          </div>
          <div
...
import ChatBotMessage from "@/viewer/agent/smartpurchase/ChatBotMessage.vue";
import SmartPurchaseApi from "@/api/SmartPurchaseApi.js";
...
const uploadedPicUrl = ref('');
const uploadedPicBase64Data = ref('');
...
const onFileSelected = async (event) => {
  ...
  try {
    ...
    uploadedPicUrl.value = imgUrlPrefix + code;
    ...
    img.src = uploadedPicUrl.value; //拿当前的图片传到后台
    ...
    uploadedPicBase64Data.value = canvas.toDataURL('image/jpeg');
...
const newSession = async () => {
  try {
    ...
    userPrompt.value = '';
    messages.value = [];
    uploadedPicUrl.value = '';
    uploadedPicBase64Data.value = '';
...
// 获取商品数据的函数
const fetchGoodsData = async (ids) => {
  let productList = [];
  if (!ids || !Array.isArray(ids) || ids.length === 0) {
    productIds.value = []; // 重置为空数组
    return;
  }
  ...
  try {
    ...
    const response = await SmartPurchaseApi.getGoodsByIdList(payload);
    productList = response;
    return productList;
  } catch (error) {
    console.error("getGoodsByIdList api :", error);
  }
  return productList;
};
...
const handlePromptSubmit = async () => {
  ...
  try {
    ...
    let params = {
      ...
      imageData: uploadedPicBase64Data.value,
      configMainId: selectedConfigMainId.value,
    ...
    ChatApi.complete(params, {
      ...
      onMessage(data) {
        ...
        // 添加调试信息确认 msg.dataIds 值
        console.log("原始 msg.dataIds:", msg.dataIds);
        ...
        if (msg.dataIds) {
          // 确保 msg.dataIds 是数组类型
          if (Array.isArray(msg.dataIds)) {
            productIds.value = [...msg.dataIds]; // 使用展开运算符创建新数组
          } else if (typeof msg.dataIds === 'string') {
            try {
              // 如果是JSON字符串，尝试解析
              productIds.value = JSON.parse(msg.dataIds);
            } catch (e) {
              // 如果不是JSON字符串且是单个值，转为数组
              productIds.value = [msg.dataIds];
            }
          } else {
            console.error("msg.dataIds 格式不正确:", msg.dataIds);
            productIds.value = [];
          }
          console.log("处理后的 productIds:", productIds.value);
          // 在这里直接调用 fetchGoodsData
          if (productIds.value.length > 0) {
            console.log("立即调用fetchGoodsData，productIds:", productIds.value);
            fetchGoodsData(productIds.value).then(productList => {
