...
@Component
public class Gpt4oComponent implements AIComponent{
    ...
    public String jsonCall(AIModel aiModel, RequestPayload requestPayload) throws Exception{
        ...
        try{
            ...
            String aiResponse = okHttpHelper.postJsonWithMultiHeaders(modelUrl, requestPayload, headers, 15000, 15000);
    ...
    public String jsonCall(AIModel aiModel,String promptStr, double temperature)throws Exception{
        try {
            ...
            logger.info(">>>>>>use Gpt4o call json");
        ...
        try {
            RealEventSource realEventSource = new RealEventSource(httpRequest, new EventSourceListener() {
                ...
                public void onFailure(EventSource eventSource, Throwable t, Response response) {
                    try {
                        logger.error(">>>>>>claude聊天访问失败->{}", response.body().string());
                    } catch (Exception e) {
                    }
                    if (callback != null) {
            ...
            logger.error(">>>>>>GPT4oComponent 访问失败->{}", e.getMessage());
            if (callback != null) {
