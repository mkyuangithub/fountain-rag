...
@Component
public class BgeVLEmbeddingRepo implements VectorDbRepo {
    ...
    @Value("${fountain.ai.embedding.vlEmbeddingUrl}")
    private String vlEmbeddingUrl = "";
    ...
    public void deleteRepoVl(String repoName)throws Exception{
        try {
            StringBuilder urlStr = new StringBuilder();
            urlStr.append(uri).append("/").append(repoName).append("_vl").append("?wait=true");
    ...
    @Override
    public List<Float> getVlEmbedding(String inputText,String imageData) throws Exception{
        List<Float> vectorResult = new ArrayList<>();
        try {
            Map<String, String> headers = new HashMap<>();
            // logger.info(">>>>>>api-key-> {}", apiKey);
            headers.put("Content-Type", "application/json");
            headers.put("apiKey", embeddingApiKey);
            Map<String, String> input = new HashMap<>();
            input.put("text", inputText);
            input.put("image", imageData);
            logger.info(">>>>>>use bgeVlEmbedding =>addr为->{}", vlEmbeddingUrl);
            String jsonStr = okHttpHelper.postJsonWithMultiHeaders(vlEmbeddingUrl, input, headers, 15000, 15000);
            ...
            if (StringUtil.isNotBlank(jsonStr)) {
                ...
                if (dataNode != null) {
                    // 从data节点中获取embeddings
                    JsonNode embeddingsNode = dataNode.get("embeddings");
                    ...
                    // 如果需要获取spentTime
                    JsonNode spentTimeNode = dataNode.get("spentTime");
                    if (spentTimeNode != null) {
                        double spentTime = spentTimeNode.doubleValue();
                        logger.info(">>>>>>BgeEmbedding spent time->{}",spentTime);
                        // 处理spentTime...
                    }
                }
            }
    ...
    public void deleteVlPoints(String collectionName,List<Long>pointIds)throws Exception{
        String deleteUrl = uri + "/" + collectionName +"_vl"+ "/points/delete" + "?wait=true";
        Map<String, String> headers = new HashMap<>();
        headers.put("api-key", apiKey);
        Map<String, Object> payload = new HashMap<>();
        payload.put("points", pointIds);
